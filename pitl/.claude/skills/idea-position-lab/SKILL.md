...
- **Mode A — Lab Template (기본, 먼저 만든다)**: 판정 로직과 문구가 `assets/engine.js`에 고정된 자기완결형 HTML 폼을 Artifact로 발행한다. 처음엔 클로드가 아이디어 설명만으로 가볍게 판단해 값을 미리 채워주지만(추측이니 확인 필요), 사용자가 "Digging"으로 직접 확인·수정하며 최종값을 확정하는 게 목적이다. 발행 후엔 AI 없이 폼 값만 바꿔가며 몇 번이고 재계산할 수 있다.
...
7. 사용자에게: 채워진 값은 클로드의 추정값이라 확인·수정이 필요하다고 안내하고, "Digging"을 누르면 결과가 나오며, 값을 확정한 뒤 "마크다운으로 내보내기"로 저장하거나 채팅에서 "Lab Report로 만들어줘"라고 하면 그 확정값 기준으로 서술형 리포트를 발행해준다고 안내한다.

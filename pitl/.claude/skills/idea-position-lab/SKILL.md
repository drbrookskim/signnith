...
description: >
  ...
  BMC, AARRR) 관점에서 진단하고 1차 전략을 제시한다. 아이디어를 문단으로 설명하며
  "분석해줘", "위치 진단해줘", "다각도로 봐줘" 등 빠른 분석을 원하면 Mode A
  (Lab Report)를, "Lab Template 만들어줘"/"나침반 만들어줘"/"계산기 만들어줘",
  "폼으로", "재사용 가능하게" 등 저장해두고 나중에 값 바꿔가며 다시 쓰고 싶다는
  요청엔 Mode B(Lab Template)를 쓴다. 기존 PITL 하네스
  (3c-analysis/4p-strategy/idea-to-strategy)와 독립적으로 동작한다.
---

# Idea Campus > Lab

아이디어를 13개 프레임워크 렌즈로 진단하고 전략을 제시하는 두 가지 모드가 있다. 결과물은 항상 "Idea Campus" 브랜드 아래 두 산출물 중 하나로 이름 붙인다 — 매번 새로 만드는 즉석 산출물은 **Idea Campus > Lab Report**, 재사용 가능한 폼 도구는 **Idea Campus > Lab Template**.

- **Mode A — Lab Report**: 아이디어 설명 한 문단만 있으면 된다. 클로드가 그 자리에서 13개 프레임워크 관점으로 직접 판단해 분석 리포트를 쓴다. 코드도 폼도 없다 — AI가 필요한 작업이니 AI가 직접 한다.
- **Mode B — Lab Template**: 판정 로직과 문구가 `assets/engine.js`에 고정된 자기완결형 HTML 폼을 Artifact로 발행한다. 발행 후엔 AI 없이 폼 값만 바꿔가며 몇 번이고 재계산할 수 있다. 아이디어를 반복해서 다듬거나 남한테 폼을 넘겨 같이 쓸 때 쓴다.

기본은 Mode A다 — 아이디어를 설명받으면 먼저 Mode A로 Lab Report를 쓰고, 사용자가 "Lab Template으로 만들어줘"/"폼으로 다듬고 싶다" 같은 요청을 하면 그때 Mode B로 넘어간다(이때 Mode A에서 이미 판단한 값을 그대로 재사용해 폼에 채워 넣는다 — 처음부터 다시 판단하지 않는다).
...
5. 리포트 맨 위에 제목 `# Idea Campus > Lab Report`를 붙인다. 분석군(위치 진단) → 전략군(1차 전략) 순서로 마크다운 리포트를 쓴다.
6. 완성된 마크다운을 `.md` 파일로 저장하고 Artifact 도구로 발행한다(파일 그대로 렌더링됨, favicon 지정, description은 아이디어 한줄 요약). 채팅 응답에는 리포트 본문 대신 핵심 요약과 발행된 URL을 안내한다 — 리포트 전체를 채팅에 다시 붙여넣지 않는다.
7. 끝에 한 줄로 안내: "이 판단값 그대로 Lab Template(폼)으로 옮겨서 다듬고 싶으면 말해달라."

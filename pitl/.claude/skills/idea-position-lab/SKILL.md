...
description: >
  ...
  진단해줘", "다각도로 봐줘" 등 빠른 분석을 원하면 Mode A(리포트)를, "나침반
  만들어줘"/"계산기 만들어줘", "폼으로", "재사용 가능하게" 등 저장해두고 나중에
  값 바꿔가며 다시 쓰고 싶다는 요청엔 Mode B(나침반)를 쓴다. 기존 PITL 하네스
...
- **Mode B — 나침반**: 판정 로직과 문구가 `assets/engine.js`에 고정된 자기완결형 HTML 폼을 Artifact로 발행한다. 발행 후엔 AI 없이 폼 값만 바꿔가며 몇 번이고 재계산할 수 있다. 아이디어를 반복해서 다듬거나 남한테 폼을 넘겨 같이 쓸 때 쓴다.

기본은 Mode A다 — 아이디어를 설명받으면 먼저 Mode A로 리포트를 쓰고, 사용자가 "나침반으로 만들어줘"/"폼으로 다듬고 싶다" 같은 요청을 하면 그때 Mode B로 넘어간다(이때 Mode A에서 이미 판단한 값을 그대로 재사용해 폼에 채워 넣는다 — 처음부터 다시 판단하지 않는다).
...
6. 끝에 한 줄로 안내: "이 판단값 그대로 나침반(폼)으로 옮겨서 다듬고 싶으면 말해달라."
